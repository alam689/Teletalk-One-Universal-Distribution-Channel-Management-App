...
/**
 * In-repo mock of the auth service. Active only when `API_BASE_URL` is unset.
 * Error `detail` values map 1:1 onto `error.*` i18n keys, exactly as the real
 * service must.
 *
 * One account per role lives in demoAccounts.ts, so every capability set in the
 * org model can be signed into and reviewed.
 *
 * Where this differs from the portal's mock, it differs because a handset is
 * not a browser. The portal's refresh token is an httpOnly cookie the client
 * cannot read; there are no cookies here, so the marker that survives a
 * restart goes to the OS keystore instead. It is the same shape of secret
 * kept in the strongest place each platform offers, which is the most that can
 * honestly be said for either.
 */
...
const SESSION_POS = VaultKeys.mockPos
const SESSION_TRUST = VaultKeys.mockTrust
...
export async function verifyOtp(
  ...
  if (!account) throw new ApiError('notFound', 404, 'posUnknown')
  try {
    vault.set(SESSION_POS, posCode)
    vault.set(SESSION_TRUST, trustDevice ? '1' : '')
  } catch {
...
export async function restoreSession(): Promise<SignInResult | null> {
  ...
  let trusted = false
  try {
    posCode = vault.get(SESSION_POS)
    trusted = vault.get(SESSION_TRUST) === '1'
  } catch {
    return null
  }
...
export async function signOut(): Promise<void> {
  await delay(150)
  try {
    vault.remove(SESSION_POS)
    vault.remove(SESSION_TRUST)
  } catch {
    /* ignore */
  }
  failedAttempts = 0
